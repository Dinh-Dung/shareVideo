...
async function login(username, password) {
    try {
        const { data, error } = await axios.post("http://localhost:8080/user/login", { username, password })
...
async function getUser() {
    try {
        const { data, error } = await axios.get("http://localhost:8080/user/getProfile")
        if (!data || error) throw new Error()
